...
public class LoginServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String ruoloUtente = null; // Sarà "privato", "azienda" o "admin"
        ...
        try {
            ...
            ps = con.prepareStatement("SELECT ID, Email, Password FROM Amministratori WHERE Email = ?");
            ...
            if (rs.next()) {
                ...
                user.setCognome("Piattaforma");
                
                ruoloUtente = "admin";
                System.out.println("DEBUG LOGIN: Trovato account AMMINISTRATORE nel DB. ID: " + user.getId());
                System.out.println("DEBUG LOGIN: Trovato account AMMINISTRATORE nel DB. Password: " + user.getPwd());
            } else {
                rs.close();
                ps.close();
                ...
                if (rs.next()) {
                    ...
                    System.out.println("DEBUG LOGIN: Trovato account PRIVATO nel DB. Password: " + user.getPwd());
                } else {
                    rs.close();
                    ps.close();
            ...
            // Verifica della password e setup della sessione
            if (user != null) {
                if (user.getPwd().equals(hashedInput)) {
                    HttpSession session = request.getSession(true);
                    session.setAttribute("utente", user);
                    session.setAttribute("tipo", ruoloUtente);
                    
                    if ("admin".equals(ruoloUtente)) {
                        System.out.println("DEBUG LOGIN: Admin autenticato. Ridirezione al pannello di controllo.");
                    ...
                    System.out.println("DEBUG LOGIN: Login autorizzato con successo per " + email + " | Tipo: " + ruoloUtente);
